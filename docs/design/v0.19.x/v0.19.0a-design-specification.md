...
9. [RealmBiomeProperties Value Object](#9-realmbiomeproperties-value-object)
10. [AdjacencyRule Value Object](#10-adjacencyrule-value-object)
11. [RealmBiomeDefinition Entity](#11-realmbiomeDefinition-entity)
12. [RealmBiomeZone Entity](#12-realmbiomezone-entity)
13. [EnvironmentalCondition Entity](#13-environmentalcondition-entity)
14. [IRealmBiomeProvider Interface](#14-ibipomeprovider-interface)
...
17. [RealmBiomeProvider Implementation](#17-realmbiomeprovider-implementation)
...
- **2 Value Objects** — RealmBiomeProperties, AdjacencyRule
- **3 Domain Entities** — RealmBiomeDefinition, RealmBiomeZone, EnvironmentalCondition
- **3 Application Interfaces** — IRealmBiomeProvider, IBiomeAdjacencyService, IEnvironmentalConditionService
- **2 Service Implementations** — RealmBiomeProvider, BiomeAdjacencyService
- **3 Configuration Files** — realm-biomes.json, realm-biomes.schema.json, adjacency-matrix.json
...
| **Value Objects**          | 2     | RealmBiomeProperties, AdjacencyRule                                         |
| **Domain Entities**        | 3     | RealmBiomeDefinition, RealmBiomeZone, EnvironmentalCondition                |
| **Application Interfaces** | 3     | IRealmBiomeProvider, IBiomeAdjacencyService, IEnvironmentalConditionService |
| **Application Services**   | 2     | RealmBiomeProvider, BiomeAdjacencyService                                   |
| **DTOs**                   | 1     | EnvironmentalDamageResult                                                   |
| **Configuration**          | 3     | realm-biomes.json, realm-biomes.schema.json, adjacency-matrix.json          |
| **Unit Tests**             | ~20   |                                                                             |
...
└── Core Biome Infrastructure
    ...
    │   ├── IRealmBiomeProvider — Biome definition access
    │   ├── IBiomeAdjacencyService — Adjacency validation
    │   └── IEnvironmentalConditionService — Hazard application
    │
    ├── Application Layer (Services)
    │   ├── RealmBiomeProvider — JSON config loading
    │   └── BiomeAdjacencyService — Adjacency matrix validation
    │
    └── Configuration
        ├── realm-biomes.json — Realm definitions
        ├── realm-biomes.schema.json — JSON Schema validation
        └── adjacency-matrix.json — Realm adjacency rules
```
...
│  │                         VALUE OBJECTS                               │     │
│  ├────────────────────────────────┬───────────────────────────────────┤     │
│  │       RealmBiomeProperties          │         AdjacencyRule             │     │
│  ├────────────────────────────────┼───────────────────────────────────┤     │
...
│  │                            ENTITIES                                 │     │
│  ├──────────────────────┬───────────────────┬─────────────────────────┤     │
│  │   RealmBiomeDefinition    │    RealmBiomeZone      │ EnvironmentalCondition  │     │
│  ├──────────────────────┼───────────────────┼─────────────────────────┤     │
...
│  │                          INTERFACES                                 │     │
│  ├──────────────────────┬───────────────────┬─────────────────────────┤     │
│  │    IRealmBiomeProvider    │ IBiomeAdjacency   │ IEnvironmental          │     │
...
│  │                       IMPLEMENTATIONS                               │     │
│  ├──────────────────────────────────┬─────────────────────────────────┤     │
│  │         RealmBiomeProvider            │      BiomeAdjacencyService      │     │
│  ├──────────────────────────────────┼─────────────────────────────────┤     │
...
│  │ + GetBiome(id) → RealmBiomeDefinition │ + GetKey(a, b) → string         │     │
│  └──────────────────────────────────┴─────────────────────────────────┘     │
│                                                                              │
│  ┌────────────────────────────────────────────────────────────────────┐     │
│  │                            DTOs                                     │     │
...
- All 9 realms load from realm-biomes.json
...
- GetBiome returns correct RealmBiomeDefinition for valid RealmId
- GetBiome returns null for invalid/unknown realm
- All RealmBiomeDefinition properties are populated
...
## 9. RealmBiomeProperties Value Object
...
**File:** `src/Core/RuneAndRust.Domain/ValueObjects/RealmBiomeProperties.cs`
...
/// <remarks>
/// <para>
/// RealmBiomeProperties define the physical characteristics of a realm that
...
/// <example>
/// <code>
/// var muspelheimProps = new RealmBiomeProperties
/// {
...
## 11. RealmBiomeDefinition Entity
...
**File:** `src/Core/RuneAndRust.Domain/Entities/RealmBiomeDefinition.cs`
...
/// <remarks>
/// <para>
/// RealmBiomeDefinition represents one of the nine canonical realms of Aethelgard.
...
/// <example>
/// <code>
/// var midgard = new RealmBiomeDefinition
/// {
///     Id = RealmId.Midgard,
///     Name = "Midgard",
...
/// </code>
/// </example>
public sealed class RealmBiomeDefinition
{
    /// <summary>
    ...
    /// </summary>
    public required RealmBiomeProperties BaseProperties { get; init; }
    ...
    /// </remarks>
    public IReadOnlyList<RealmBiomeZone> Zones { get; init; } = [];
...
**File:** `src/Core/RuneAndRust.Domain/Entities/RealmBiomeZone.cs`
...
/// <remarks>
/// <para>
/// RealmBiomeZone represents a distinct area within a realm that may have
...
/// <example>
/// <code>
/// var forgeCore = new RealmBiomeZone
/// {
///     Id = "forge-core",
...
/// </code>
/// </example>
public sealed class RealmBiomeZone
{
    /// <summary>
    ...
    /// </remarks>
    public RealmBiomeProperties? OverrideProperties { get; init; }
...
## 14. IRealmBiomeProvider Interface
...
**File:** `src/Core/RuneAndRust.Application/Interfaces/IRealmBiomeProvider.cs`
...
/// <remarks>
/// <para>
/// IRealmBiomeProvider abstracts the loading and retrieval of biome data,
...
## 17. RealmBiomeProvider Implementation
...
**File:** `src/Infrastructure/RuneAndRust.Infrastructure/Services/RealmBiomeProvider.cs`
...
{
    private readonly Dictionary<RealmId, RealmBiomeDefinition> _biomes;
    ...
    public RealmBiomeProvider(IConfigurationProvider configProvider, ILogger<RealmBiomeProvider> logger)
    {
        _logger = logger ?? throw new ArgumentNullException(nameof(logger));

        // Load configuration
        var config = configProvider.LoadConfig<RealmBiomeConfiguration>("realm-biomes.json")
            ?? throw new InvalidOperationException("Failed to load realm-biomes.json");
...
public sealed class BiomeAdjacencyService : IBiomeAdjacencyService
{
        ...
        ILogger<BiomeAdjacencyService> logger)
    {
        ...
        var rules = configProvider.LoadConfig<List<AdjacencyRule>>("adjacency-matrix.json")
            ?? GetDefaultRules();
...
### 20.1 realm-biomes.json (Starter Template)

**File:** `config/realm-biomes.json`

See the complete JSON configuration in the scope breakdown document. Key structure:

```json
{
    "$schema": "./schemas/realm-biomes.schema.json",
    "biomes": [
        {
            "id": "Midgard",
            "name": "Midgard",
...
### 21.1 RealmBiomeProvider Logging

| Event               | Level       | Template                                                                             |
| ------------------- | ----------- | ------------------------------------------------------------------------------------ |
| Initialization      | Information | `"RealmBiomeProvider initialized: {BiomeCount} biomes, {ConditionCount} conditions"` |
...
### 22.1 RealmBiomeProvider Tests (~6 tests)

**File:** `tests/RuneAndRust.Infrastructure.UnitTests/Services/RealmBiomeProviderTests.cs`
...
- [ ] `src/Core/RuneAndRust.Domain/ValueObjects/RealmBiomeProperties.cs`
- [ ] `src/Core/RuneAndRust.Domain/ValueObjects/AdjacencyRule.cs`
- [ ] `src/Core/RuneAndRust.Domain/Entities/RealmBiomeDefinition.cs`
- [ ] `src/Core/RuneAndRust.Domain/Entities/RealmBiomeZone.cs`
- [ ] `src/Core/RuneAndRust.Domain/Entities/EnvironmentalCondition.cs`

### 23.2 Application Layer

- [ ] `src/Core/RuneAndRust.Application/Interfaces/IRealmBiomeProvider.cs`
...
- [ ] `src/Infrastructure/RuneAndRust.Infrastructure/Services/RealmBiomeProvider.cs`
- [ ] `src/Infrastructure/RuneAndRust.Infrastructure/Services/BiomeAdjacencyService.cs`

### 23.4 Configuration

- [ ] `config/realm-biomes.json`
- [ ] `config/schemas/realm-realm-biomes.schema.json`
- [ ] `config/adjacency-matrix.json`

### 23.5 Tests

- [ ] `tests/RuneAndRust.Infrastructure.UnitTests/Services/RealmBiomeProviderTests.cs`
...
- [ ] RealmBiomeDefinition stores all required properties from design docs
- [ ] RealmBiomeZone supports property overrides and DC modifiers
- [ ] EnvironmentalCondition defines all hazard types with mechanics
- [ ] IRealmBiomeProvider loads and retrieves biome definitions
...
| RealmBiomeDefinition           | v0.19.1-v0.19.6 | Realm data structure |
| IRealmBiomeProvider            | Room generation | Biome retrieval      |
