...
public class GameView
{
    ...
    {
        _logger.LogDebug("Displaying abilities list");
        await _renderer.RenderMessageAsync("Abilities display not yet implemented.", MessageType.Info, ct);
    }

    private async Task HandleUseAbilityAsync(string abilityName, CancellationToken ct)
    {
        _logger.LogDebug("Using ability: {AbilityName}", abilityName);
        await _renderer.RenderMessageAsync($"Ability '{abilityName}' usage not yet implemented.", MessageType.Info, ct);
    }
}
