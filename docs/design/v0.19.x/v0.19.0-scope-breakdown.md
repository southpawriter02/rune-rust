# v0.19.0 Core Biome Infrastructure - Scope Breakdown

**Version:** 0.19.0
**Theme:** Core Biome Infrastructure
**Prerequisites:** v0.18.x Complete (Advanced Character Creation System)
...
v0.19.0 establishes the foundational infrastructure for the Nine Realms biome system. This version defines the core entities, enumerations, and provider interfaces that all subsequent realm implementations will utilize. The work is divided into a single foundational phase that provides the building blocks for v0.19.1-v0.19.6.

The work is divided into **1 sub-phase**:

| Phase    | Name                      | Focus                                          | Est. Tests |
| -------- | ------------------------- | ---------------------------------------------- | ---------- |
| v0.19.0a | Core Biome Infrastructure | Enums, entities, interfaces, provider services | ~20        |
...
| ------------------------------- | ---------- | ---------------------------------------------------- | -------------- |
| RealmId Enum                    | Low        | None                                                 | **v0.19.0a**   |
| BiomeCompatibility Enum         | Low        | None                                                 | **v0.19.0a**   |
| EnvironmentalConditionType Enum | Low        | None                                                 | **v0.19.0a**   |
| VerticalZone Enum               | Low        | None                                                 | **v0.19.0a**   |
| BiomeProperties Value Object    | Medium     | None                                                 | **v0.19.0a**   |
| AdjacencyRule Value Object      | Medium     | RealmId, BiomeCompatibility                          | **v0.19.0a**   |
| BiomeDefinition Entity          | Medium     | RealmId, BiomeProperties, EnvironmentalConditionType | **v0.19.0a**   |
| BiomeZone Entity                | Medium     | RealmId, BiomeProperties                             | **v0.19.0a**   |
| EnvironmentalCondition Entity   | Medium     | EnvironmentalConditionType                           | **v0.19.0a**   |

### From v0.19.x-overview.md - Application Layer

| Feature                                  | Complexity | Dependencies                               | Assigned Phase |
| ---------------------------------------- | ---------- | ------------------------------------------ | -------------- |
| IBiomeProvider Interface                 | Medium     | BiomeDefinition, BiomeZone                 | **v0.19.0a**   |
| IBiomeAdjacencyService Interface         | Medium     | RealmId, BiomeCompatibility, AdjacencyRule | **v0.19.0a**   |
| IEnvironmentalConditionService Interface | Medium     | EnvironmentalCondition                     | **v0.19.0a**   |
| BiomeProvider Implementation             | Medium     | IBiomeProvider                             | **v0.19.0a**   |
| BiomeAdjacencyService Implementation     | Medium     | IBiomeAdjacencyService                     | **v0.19.0a**   |

### From v0.19.x-overview.md - Infrastructure Layer

| Feature                   | Complexity | Dependencies           | Assigned Phase |
| ------------------------- | ---------- | ---------------------- | -------------- |
| biomes.json Configuration | Medium     | BiomeDefinition schema | **v0.19.0a**   |
| biomes.schema.json        | Medium     | None                   | **v0.19.0a**   |
| adjacency_matrix.json     | Low        | AdjacencyRule schema   | **v0.19.0a**   |
...
## v0.19.0a: Core Biome Infrastructure

[v0.19.0a Design Specification](v0.19.0a-design-specification.md)

### Overview

This phase establishes the complete foundational infrastructure for the Nine Realms biome system, including domain enumerations, value objects, entities, application layer interfaces, and JSON configuration schemas.

### Scope

**In Scope:**

- `RealmId` enum (9 canonical realms)
- `BiomeCompatibility` enum (Compatible, RequiresTransition, Incompatible)
- `EnvironmentalConditionType` enum (9 hazard types)
- `VerticalZone` enum (8 Z-level classifications)
- `BiomeProperties` value object (temperature, aetheric intensity, humidity, light, scale, corrosion)
- `AdjacencyRule` value object (realm pair relationships)
- `BiomeDefinition` entity (core realm definition)
- `BiomeZone` entity (sub-biome within a realm)
- `EnvironmentalCondition` entity (hazard with DC and damage)
- `IBiomeProvider` interface and implementation
- `IBiomeAdjacencyService` interface and implementation
- `IEnvironmentalConditionService` interface
- JSON configuration and schema files

**Out of Scope:**

- Individual realm implementations (v0.19.1+)
- Realm-specific environmental conditions (v0.19.1+)
- Faction spawn pool integration (v0.19.1+)
- Room template filtering by biome (v0.20.x)
- Travel time calculations (v0.19.6)
- Dungeon sector biome assignment (v0.19.6)
...
| Domain Value Objects   | 2     | BiomeProperties, AdjacencyRule                                         |
...
└── Helheim = 9 (Deck 08 — Waste Reclamation)

NEW: BiomeCompatibility (Enum)
├── Compatible
├── RequiresTransition
└── Incompatible

NEW: EnvironmentalConditionType (Enum)
├── None
├── IntenseHeat
├── ExtremeCold
├── ToxicAtmosphere
...
NEW: BiomeProperties (Value Object)
├── TemperatureCelsius: int
├── AethericIntensity: float
├── HumidityPercent: int
├── LightLevel: float
├── ScaleFactor: float
└── CorrosionRate: float

NEW: AdjacencyRule (Value Object)
├── RealmA: RealmId
├── RealmB: RealmId
├── Compatibility: BiomeCompatibility
├── MinTransitionRooms: int
├── MaxTransitionRooms: int
└── TransitionTheme: string?

NEW: BiomeDefinition (Entity)
├── Id: RealmId
├── Name: string
├── Subtitle: string
├── DeckNumber: int
├── PreGlitchFunction: string
├── PostGlitchState: string
├── BaseProperties: BiomeProperties
├── PrimaryCondition: EnvironmentalConditionType
├── MinVerticalZone: VerticalZone
├── MaxVerticalZone: VerticalZone
├── Zones: IReadOnlyList<BiomeZone>
├── FlavorQuote: string?
└── ColorPalette: string?

NEW: BiomeZone (Entity)
├── Id: string
├── Name: string
├── ParentRealm: RealmId
├── OverrideProperties: BiomeProperties?
├── OverrideCondition: EnvironmentalConditionType?
├── ConditionDcModifier: int
├── DamageOverride: string?
├── MinVerticalZone: VerticalZone?
└── MaxVerticalZone: VerticalZone?

NEW: EnvironmentalCondition (Entity)
...
├── Frequency: string
├── Description: string
└── Mitigations: IReadOnlyList<string>
```
...
```
N/A — Infrastructure version; no new player commands
```
...
v0.18.x (Advanced Character Creation) - REQUIRED
    │
    └── Character stats for environmental checks
                                    │
                                    ▼
v0.19.0 (Core Biome Infrastructure)
    │
    └── v0.19.0a: Core Infrastructure ─────────────────────────┐
            Dependencies: DamageService (v0.6.x), CharacterState │
                                                                 │
                                                                 ▼
v0.19.1-v0.19.6 (Realm Implementations)
    │
    ├── REQUIRES: RealmId enum
    ├── REQUIRES: BiomeDefinition entity
    ├── REQUIRES: IBiomeProvider
    └── REQUIRES: IBiomeAdjacencyService
```
...
| ------------------------------------- | ------ | ----------- | ------------------------------------- |
| Adjacency rules become complex        | Medium | Low         | Pre-define matrix in configuration    |
| Environmental conditions scale poorly | Medium | Low         | Use provider pattern for lazy loading |
| Schema validation failures            | Low    | Medium      | Thorough test coverage on loader      |
...
| ------------------------- | ---------------------- | ------------------------------------------------ |
| **Realm identifier type** | Enum (RealmId)         | Fixed set of 9 realms; enum provides type safety |
| **Deck numbering**        | 1-based integers (1-9) | Matches YGGDRASIL Network designation            |

### Adjacency System

| Decision                        | Value                                      | Notes                                               |
| ------------------------------- | ------------------------------------------ | --------------------------------------------------- |
| **Adjacency storage**           | Configuration file (adjacency_matrix.json) | Allows modification without code changes            |
| **Incompatibility enforcement** | Service-level validation                   | IBiomeAdjacencyService validates all realm pairings |
| **Muspelheim ↔ Niflheim**       | Incompatible                               | Fire/Ice contradiction                              |
| **Muspelheim ↔ Vanaheim**       | Incompatible                               | Fire/Bio contradiction                              |

### Environmental Conditions

| Decision              | Value                          | Notes                                      |
| --------------------- | ------------------------------ | ------------------------------------------ |
| **Base DC**           | 12 for most conditions         | Standard difficulty baseline               |
| **Zone DC modifiers** | -4 to +8 range                 | Allows significant zone-specific variation |
| **Damage scaling**    | Per-zone damage dice overrides | More dangerous zones deal more damage      |
...
2. **v0.19.0a Design Spec** - Create detailed design specification
3. **Implement v0.19.0a** - Build core infrastructure
4. **Proceed to v0.19.1** - Implement Midgard (first realm)
...
_This scope breakdown provides a structured approach to implementing v0.19.0 Core Biome Infrastructure. This foundational phase establishes all building blocks required for v0.19.1-v0.19.6 realm implementations._
