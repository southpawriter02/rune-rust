...
1. **Route Skar-Horde abilities** through CombatEngine
2. **Implement Savagery generation** from hits/damage taken
3. **Add Savagery bar** to CombatView
4. **Implement augment type requirements** for abilities

### Phase 3: Status Effects

1. **Define status effects** ([Grievous Wound], [Rooted], [Apotheosis])
2. **Implement Soak bypass** for Grievous Wound
3. **Implement immunity system** (Fear, Stun during Apotheosis)

### Phase 4: Trauma Integration

1. **Implement Psychic Stress generation** from The Price of Power
2. **Implement post-Apotheosis Stress** penalty
3. **Add trauma warning indicators** to GUI

### Phase 5: Polish

1. **Add rank-specific icons** (Bronze/Silver/Gold)
2. **Implement rank-up notifications**
3. **Add Apotheosis transformation notification**
