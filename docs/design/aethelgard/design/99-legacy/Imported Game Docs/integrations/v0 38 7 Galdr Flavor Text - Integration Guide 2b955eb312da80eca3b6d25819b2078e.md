...
1. **✅ Ability Class Extended** (`RuneAndRust.Core/Ability.cs`)
...
2. **✅ CombatEngine Extended** (`RuneAndRust.Engine/`)
...
3. **✅ AbilityDatabase Extended** (`RuneAndRust.Engine/AbilityDatabase_GaldrExtensions.cs`)
...
- **Warrior abilities** → `AbilityCategory` (WeaponArt, TacticalAbility, etc.)
- **Mystic abilities** → `RuneSchool` + `Element` (when you add them)
...
> "Fire engulfs the Corrupted Servitor, scorching their chassis! Circuits melt, hydraulics burst—it jerks spasmodically! [15 damage]"
> 
...
> "Your chant falters—the Blight warps Fehu's meaning! Fire burns backward, cold as ice, wrong!"
> 
...
[Outcome] Fire washes over the Servitor's chassis! Circuits melt, hydraulics burst—
...
**May your Galdr resonate with the ancient powers!** 🔥⚡❄️🌿