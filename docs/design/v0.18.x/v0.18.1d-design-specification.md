...
- [x] `src/Core/RuneAndRust.Domain/ValueObjects/CorruptionTransferResult.cs`
- [x] `src/Core/RuneAndRust.Domain/ValueObjects/CorruptionSkillModifiers.cs`

### Application Layer — Service

- [x] `src/Core/RuneAndRust.Application/Services/CorruptionService.cs`

### Application Layer — Repository Interface (if not exists)

- [x] `src/Core/RuneAndRust.Application/Interfaces/ICorruptionRepository.cs`

### Unit Tests

- [x] `tests/RuneAndRust.Application.UnitTests/Services/CorruptionServiceTests.cs`

### Documentation

- [x] XML documentation on all public members
- [x] Code examples in remarks sections
- [x] Update scope breakdown to mark v0.18.1d complete
...
- [x] `AddCorruption` correctly delegates to tracker and persists
- [x] `AddCorruption` logs at appropriate levels (Info/Warning/Error)
- [x] `TransferCorruption` atomically updates both trackers
- [x] `TransferCorruption` fails gracefully if source has insufficient corruption
- [x] `TransferCorruption` detects Terminal Error on target
- [x] `RemoveCorruption` requires valid reason (logged at Warning)
- [x] `PerformTerminalErrorCheck` uses WILL minus Resolve penalty
- [x] `PerformTerminalErrorCheck` sets corruption to 99 on survival
- [x] `GetSkillModifiers` returns correct Stage-based values
- [x] `GetSkillModifiers.FactionLocked` is true when corruption ≥ 50
...
- [x] All log messages use structured logging with `{Property}` placeholders
