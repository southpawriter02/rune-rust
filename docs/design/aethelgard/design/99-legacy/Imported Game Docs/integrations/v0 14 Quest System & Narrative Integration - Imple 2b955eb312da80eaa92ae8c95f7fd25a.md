...
✅ **Enhanced Quest Data Model** - Structured quest representation with objectives and rewards
✅ **Quest State Machine** - Track quest progress through states (Available → Active → Complete → TurnedIn)
✅ **Quest Anchor Integration** - Procedural generation guarantees quest locations appear
✅ **Objective Tracking** - Multiple objective types (kill, collect, explore, interact)
✅ **Quest Rewards** - Experience, items, reputation, unlocked content
✅ **Quest Prerequisites** - Quest chains with Legend level requirements
✅ **Dynamic Quest Generation** - Procedural quests adapt to generated content
✅ **Quest Journal UI** - Enhanced quest viewing with detailed progress tracking
...
├── ObjectiveTypes.cs                    # Enhanced objective classes (Kill, Collect, Explore, Interact)
├── QuestGenerationRequirements.cs       # Quest-driven dungeon generation

RuneAndRust.Engine/
├── DynamicQuestGenerator.cs             # Procedural quest generation

Data/Quests/
├── quest_schematic_recovery_v014.json   # Example v0.14 quest with all features
...
```
RuneAndRust.Core/Quests/
├── Quest.cs                   # Added: Type, Category, Prerequisites, Generation Reqs, Timestamps
├── QuestReward.cs             # Added: Multiple factions, item details, unlocks

RuneAndRust.Engine/
├── QuestService.cs            # Added: OfferQuest, TurnInQuest, state queries, prerequisite checking
...
```
NotStarted → Available → Active → Complete → TurnedIn
                  ↓         ↓
              Abandoned   Failed
...
1. **NotStarted → Available**: Quest offered via `QuestService.OfferQuest()`
2. **Available → Active**: Player accepts quest via `QuestService.AcceptQuest()`
3. **Active → Complete**: All objectives completed (auto-detected via `UpdateQuestProgress()`)
4. **Complete → TurnedIn**: Player turns in quest to NPC via `QuestService.TurnInQuest()`
...
```csharp
{
    "Description": "Find the Jötun-Reader Archive",
    "Type": "ExploreRoom",
...
- **XP**: `roomCount × multiplier` (30-50 per room)
- **Cogs**: `roomCount × multiplier` (20-40 per room)
- **Reputation**: Fixed bonuses (10-15 points)

---

## 📊 Quest Queries & Management
...
1. NPC offers quest → Quest becomes **Available**
2. Player accepts → Quest becomes **Active**
3. Complete objectives → Quest becomes **Complete**
4. Return to NPC → Quest becomes **TurnedIn**, rewards granted

### Example Quest Flow

```
[Player explores The Roots]
[Finds Kári the Tinkerer in settlement]

Player: "talk kári"
Kári: "I need engineering schematics from the old archives..."
      [Quest offered: Schematic Recovery]

Player: "accept quest schematic_recovery"
[Quest accepted: Schematic Recovery]
[Objectives: Find Jötun-Reader Archive (0/1)]

[Player enters Quest Anchor room]
[Quest objective complete: Find Jötun-Reader Archive (1/1)]
...
[Quest Complete: Schematic Recovery - Return to Kári for reward!]

Player: "talk kári"
Kári: "Excellent work! Here's your reward."
...
- Example: "Find the Jötun-Reader Archive" requires `jotun_reader_archive` anchor
...
1. [**Enemy.Id](http://enemy.id/) Property**: Program.cs calls `enemy.Id` but Enemy class uses `enemy.Type`. This needs to be fixed to `enemy.Type.ToString()`.
...
1. **Create Quest Anchor** (if needed) in `Data/QuestAnchors/`
2. **Create Handcrafted Room** (if needed) in `Data/HandcraftedRooms/`
3. **Test**: Load game, find NPC, accept quest, complete objectives
...
- Available → Active → Complete → TurnedIn flow implemented
