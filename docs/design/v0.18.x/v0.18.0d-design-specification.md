...
- [x] `src/Core/RuneAndRust.Application/Services/StressService.cs`

### Unit Tests

- [x] `tests/RuneAndRust.Application.UnitTests/Services/StressServiceTests.cs`

### Documentation

- [x] XML documentation on class and all methods
...
- [x] `ApplyStress` with `resistDc > 0` performs WILL check via `IDiceService`
- [x] Successful resistance reduces stress per 0/50/75/100% table
- [x] Failed resistance (0 successes) applies full stress
- [x] `ThresholdCrossed` detected on threshold boundary crossing
- [x] `TraumaCheckTriggered` true only when reaching exactly 100
- [x] `RecoverStress(Short)` uses WILL × 2 formula
- [x] `RecoverStress(Long)` uses WILL × 5 formula
- [x] `RecoverStress(Sanctuary)` fully resets to 0
- [x] `RecoverStress(Milestone)` recovers fixed 25
- [x] `ResetAfterTraumaCheck(true)` sets stress to 75
- [x] `ResetAfterTraumaCheck(false)` sets stress to 50

### Quality Criteria

- [x] Build succeeds with 0 errors
- [x] Build succeeds with 0 warnings
- [x] All ~8 unit tests pass (85 actual)
- [x] Structured logging implemented for all operations
