...
1. **Initialization:** `StartCombat` assigns default rows (Warriors/Skirmishers → Front, Mystics/Adepts → Back).
...
| Valid Check | Trace | "Target validation: {Attacker} vs {Target} → {Result}." | Attacker, Target, Result |
...
4. **Append Status:** If `StatusApplied` not null → `" {Target} is now [{StatusColor}]{Status}[/]."`
