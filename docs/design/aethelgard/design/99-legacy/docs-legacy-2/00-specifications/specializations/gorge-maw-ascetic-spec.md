...
4. **Implement Push mechanics** - Front to Back row movement
5. **Implement zone creation** - Difficult Terrain with effects
6. **Route abilities** through CombatEngine

### Phase 3: Status Effects
7. **Define status effects** ([Rooted], [Stunned], [Knocked Down])
8. **Implement control effect durations**
9. **Implement mental immunity** (Inner Stillness)

### Phase 4: Capstone & Polish
10. **Implement Earthshaker** - Permanent terrain creation
11. **Add flying enemy detection** and warnings
12. **Add terrain visual effects**
