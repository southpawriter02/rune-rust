> **Archived** - This changelog has been consolidated. See the complete version at [v0.3.8](../v0.3.x/v0.3.8.md).

# v0.3.8 - Dynamic Room Engine Implementation
...
**Key Achievements:**
- ✅ 20 JSON room templates + 1 biome definition (the_roots) persisted to PostgreSQL
- ✅ 3 new database tables with JSONB mapping for complex data structures
- ✅ Variable substitution engine (`{Adjective}`, `{Detail}`) for dynamic content
- ✅ 6 spawn rules implemented (NeverInEntryHall, OnlyInLargeRooms, RequiredArchetype, etc.)
- ✅ Weighted random selection with adjusted weights for secret rooms
- ✅ Full backwards compatibility via deprecated GenerateTestMapAsync()
- ✅ 80%+ test coverage across 3 new test suites (46 tests total)
...
**Key Columns:**
- `Id` (UUID, PK)
- `BiomeId` (VARCHAR(100), FK → BiomeDefinitions.BiomeId)
...
- `MapBiomeIdToBiomeType(string biomeId)` - String → enum mapping (lines 273-279)
- `MapDifficultyToDangerLevel(string difficulty)` - String → enum mapping (lines 285-292)
...
```
1. DormantProcess (Enemies) → Updates context.SpawnedEnemyTypes
2. DynamicHazard (Hazards) → Updates context.SpawnedHazardTypes
3. AmbientCondition (Conditions) → Assigns room.ConditionId
```

**Phase 1 Implementation Status:**
- ✅ DynamicHazard spawning (lines 319-322)
- ✅ AmbientCondition spawning (lines 334-336)
- ⏳ DormantProcess (enemies) - logged TODO (lines 313-317)
- ⏳ StaticTerrain - logged TODO (lines 324-327)
- ⏳ LootNode - logged TODO (lines 329-332)
...
**Test Count:** 8
**Coverage:**
- ✅ {Adjective} token substitution in names
- ✅ Multiple name template selection
- ✅ Empty template handling
- ✅ {Adjective} and {Detail} substitution in descriptions
- ✅ Atmospheric detail appending (sounds and smells)
- ✅ Adjective lowercase conversion for mid-sentence usage
...
**Test Count:** 17
**Coverage:**
- ✅ NeverInEntryHall rule (blocks and allows)
- ✅ NeverInBossArena rule
- ✅ OnlyInLargeRooms rule (blocks small, allows large)
- ✅ RequiredArchetype rule (mismatch and match)
- ✅ RequiresRoomNameContains rule (no match and match)
- ✅ Multiple rules combined (all pass and one fails)
- ✅ GetAdjustedWeight with no modifiers
- ✅ HigherWeightInSecretRooms multiplier (specified and default)
- ✅ Weight multiplier only in secret rooms

#### DungeonGeneratorTests.cs
**Location:** `RuneAndRust.Tests/Engine/DungeonGeneratorTests.cs`
**Test Count:** 15
**Coverage:**
- ✅ BiomeNotFound exception
- ✅ Correct room count generation
- ✅ First room is EntryHall at (0,0,0)
- ✅ Last room is BossArena at correct position
- ✅ Bidirectional room linking (North/South)
- ✅ EnvironmentPopulator called for each room
- ✅ BiomeType mapping (the_roots → Industrial)
- ✅ Difficulty to DangerLevel mapping (Easy→Safe, Medium→Moderate, Hard→Dangerous)
- ✅ ClearAllRoomsAsync called before generation
- ✅ Returns starting room ID
- ✅ GetOppositeDirection utility method (all 6 directions)

### Integration Tests (1 Test Suite, 6 Tests)

#### TemplateLoadingIntegrationTests.cs
**Location:** `RuneAndRust.Tests/Integration/TemplateLoadingIntegrationTests.cs`
**Test Count:** 6
**Coverage:**
- ✅ Load valid template and persist to database
- ✅ JSONB roundtrip integrity (complex arrays survive serialization/deserialization)
- ✅ Load valid biome definition with nested DescriptorCategories
- ✅ Biome element loading from nested JSON structure
- ✅ Upsert logic (update existing template without creating duplicate)
- ✅ Nested ElementSpawnRules roundtrip integrity
...
### New Repository Registrations (lines 55-58)
```csharp
...
### New Service Registrations (lines 147-150)
```csharp
...
### Seeding Integration (lines 169-170)
```csharp
...
**Service Interfaces:**
5. `RuneAndRust.Engine/Services/ITemplateLoaderService.cs`
...
- Linear northward progression (EntryHall → Corridors/Chambers → BossArena)
...
Room 2 (0,1,0): "Corridor of Forgotten Memories" [Corridor, Moderate]
  → South to Room 1, North to Room 3
  → Hazard: Steam Vent (spawned via BiomeElement rule)

Room 3 (0,2,0): "The Echoing Archive" [Chamber, Moderate]
  → South to Room 2, North to Room 4
  → Condition: Radiation Aura

Room 4 (0,3,0): "Narrow Maintenance Tunnel" [Corridor, Moderate]
  → South to Room 3, North to Room 5

Room 5 (0,4,0): "The Pulsing Reactor Core" [BossArena, Dangerous]
  → South to Room 4
  → Atmospheric: "You hear the reactor's heartbeat pulsing through the walls"
```
...
- [Dynamic Room Engine Architecture](../architecture/dynamic-room-engine.md) *(TODO: Create)*
- [Spawn Rule System Guide](../guides/spawn-rules.md) *(TODO: Create)*
- [Template JSON Format Specification](../specs/template-format-spec.md) *(TODO: Create)*
- [AAM-VOICE Compliance for Templates](../guides/aam-voice-templates.md) *(TODO: Create)*
