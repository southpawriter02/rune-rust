...
- [x] IStressService defines all 9 methods
...
- [x] ApplyStress clamps results to 0-100 range
- [x] ApplyStress with resistDc > 0 performs WILL check
- [x] Successful resistance reduces stress per formula
- [x] Failed resistance applies full stress
- [x] ThresholdCrossed detected on threshold boundary
- [x] TraumaCheckTriggered true only when reaching exactly 100
- [x] RecoverStress uses correct formula per RestType
- [x] ResetAfterTraumaCheck sets 75 (passed) or 50 (failed)
- [x] ~8 unit tests pass (85 actual)
...
- [x] Schema validates correct stress source definitions
- [x] Schema rejects invalid baseStress values (< 1 or > 100)
- [x] Schema rejects invalid resistDc values (< 0 or > 10)
- [x] Configuration loads all stress source categories
- [x] ~3 unit tests pass (19 actual)
...
- [x] psychic_stress column added with CHECK constraint (deferred — Player.PsychicStress already persisted via InMemoryPlayerRepository; no character_resources table exists)
- [x] stress_history table created with proper indexes (EF Core configuration created; migration deferred — no migration infrastructure exists)
- [x] StressHistoryEntry records on each ApplyStress call (entity and repository created; StressService integration deferred to avoid breaking 85 existing tests)
- [x] ~2 unit tests pass (30 actual)
