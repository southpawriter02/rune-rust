...
| `RuneAndRust.Tests/Terminal/CombatGridRendererIntentTests.cs` | 9 unit tests validating intent icon rendering (⚔️/🛡️/💨/💤/?), status icon display (🩸/🤢/💫/💔/🛡️), stack count formatting, and combined intent/status/targeting display |
...
| `RuneAndRust.Core/ViewModels/CombatViewModel.cs` | Added `IntentIcon` parameter to `CombatantView` record (type `string?`) showing enemy intent icons (⚔️/🛡️/💨/💤/? or null for player). Added `StatusIcons` parameter (type `string?`) showing Unicode status effect icons with stack counts |
...
**StatusIcons Format:**
- Single effect: `"🩸"` (Bleeding)
- Stacked effect: `"🩸×3"` (Bleeding with 3 stacks)
- Multiple effects: `"🩸 🤢"` (Bleeding and Poisoned, space-separated)
- No effects: `null`
...
**Edge Cases:**
- `null` action → `"?"`
- `isRevealed = false` → `"?"` regardless of action
- Unknown `ActionType` → `"?"` (defensive fallback)
...
3. For stacks > 1, append `×{Stacks}` to icon
4. Join icons with space separator
5. Return `null` if no icons generated (all effects unmapped)

**Output Examples:**
- `"🩸×2"` - Bleeding with 2 stacks
- `"🩸 🤢×3"` - Bleeding (1 stack) and Poisoned (3 stacks)
- `"💫"` - Stunned (non-stacking effect)
...
**Example Outputs:**
- ` [red]Voidling[/] [yellow]⚔️[/] 🩸×2` - Enemy with revealed Attack intent and Bleeding ×2
- `>[red]Feralhound[/] !!! [yellow]?[/] 💫` - Active enemy, Critical health, hidden intent, Stunned
- ` [cyan]Kael[/] 🛡️ 🤢` - Player with Fortified and Poisoned (no intent icon)
...
| `GetViewModel_Enemy_WithRevealedIntent_ShowsIntentIcon` | Asserts `CombatantView.IntentIcon` shows action icon (⚔️) when `IsIntentRevealed = true` |
...
| `GetViewModel_DefendAction_ShowsShieldIcon` | Tests Defend action maps to `"🛡️"` icon |
...
| Test Name | Description |
|-----------|-------------|
| `FormatCombatant_Enemy_WithIntentIcon_ShowsIcon` | Verifies intent icon (⚔️) appears in formatted output for enemies |
...
| `FormatCombatant_Enemy_WithDefendIntent_ShowsShieldIcon` | Tests Defend icon (🛡️) renders correctly |
| `FormatCombatant_WithStatusIcons_ShowsIcons` | Validates status icons (🩸 🤢) appear in output |
| `FormatCombatant_WithStackedStatus_ShowsStackCount` | Confirms stacked status displays as `🩸×3` |
...
│   ├── Rendering/
│   │   └── CombatGridRenderer.cs [MODIFIED]
│   └── Program.cs [MODIFIED]
└── RuneAndRust.Tests/
    ├── Engine/
    │   ├── AnalyzedStatusTests.cs [NEW]
    │   ├── CombatServiceTests.cs [MODIFIED - IDiceService mock added]
    │   ├── IntentSystemTests.cs [NEW]
    │   ├── RowAssignmentTests.cs [MODIFIED - IDiceService mock added]
    │   └── TimelineProjectionTests.cs [MODIFIED - IDiceService mock added]
    └── Terminal/
        └── CombatGridRendererIntentTests.cs [NEW]
```
