...
public class GameSessionService
{
    ...
    {
        _logger.LogTrace("Starting new game for player: {PlayerName} Context: {Context}", playerName, context);
    ...
    {
        _logger.LogTrace("Loading game session: {SessionId} Context: {Context}", sessionId, context);
    ...
    {
        _logger.LogTrace("TryMove called with direction: {Direction} Context: {Context}", direction, context);

        if (_currentSession == null)
        {
            _logger.LogWarning("TryMove failed: No active game session Context: {Context}", context);
        ...
        if (currentRoom == null)
        {
            _logger.LogError("TryMove failed: Current room is null for session {SessionId} Context: {Context}", _currentSession.Id, context);
        ...
        {
            _logger.LogDebug(
                "Move blocked: No exit {Direction} from room {RoomName}. Available exits: {Exits} Context: {Context}",
                direction,
                fromRoom,
        ...
        if (_currentSession.TryMovePlayer(direction))
        {
            ...
            _logger.LogInformation(
                "Player moved {Direction}: {FromRoom} ({FromX},{FromY}) -> {ToRoom} ({ToX},{ToY}) Context: {Context}",
                direction,
                fromRoom,
                fromPosition.X,
                fromPosition.Y,
        ...
        _logger.LogWarning("TryMove failed unexpectedly for direction {Direction} Context: {Context}", direction, context);
    ...
    {
        _logger.LogTrace("TryPickUpItem called for item: {ItemName} Context: {Context}", itemName, context);

        if (_currentSession == null)
        {
            _logger.LogWarning("TryPickUpItem failed: No active game session Context: {Context}", context);
            return (false, "No active game session.");
        }
        ...
        if (currentRoom == null)
        {
            _logger.LogError("TryPickUpItem failed: Current room is null for session {SessionId} Context: {Context}", _currentSession.Id, context);
            return (false, "Error: Current room not found.");
        }
        ...
        {
            _logger.LogDebug(
                "Item not found in room: {ItemName}. Available items: {AvailableItems} Context: {Context}",
                itemName,
        ...
        {
            _logger.LogWarning(
                "Inventory full: Cannot pick up {ItemName}. Inventory: {Count}/{Capacity} Context: {Context}",
                itemName,
                inventoryBefore,
                _currentSession.Player.Inventory.Capacity,
                context);
        ...
        {
            _logger.LogInformation(
                "Item picked up: {ItemName} (Type: {ItemType}, Value: {ItemValue}). Inventory: {Before} -> {After}/{Capacity} Context: {Context}",
                item.Name,
                item.Type,
                item.Value,
                inventoryBefore,
    ...
    {
        _logger.LogTrace("TryAttack called Context: {Context}", context);

        if (_currentSession == null)
        {
            _logger.LogWarning("TryAttack failed: No active game session Context: {Context}", context);
        ...
        if (currentRoom == null)
        {
            _logger.LogError("TryAttack failed: Current room is null for session {SessionId} Context: {Context}", _currentSession.Id, context);
        ...
        _logger.LogDebug(
            ...
            "Monster: {MonsterName} (HP:{MonsterHealth}, ATK:{MonsterAtk}, DEF:{MonsterDef}) Context: {Context}",
            _currentSession.Player.Name,
            playerHealthBefore,
        ...
        _logger.LogInformation(
            ...
            "Player HP: {PlayerBefore} -> {PlayerAfter}, Monster HP: {MonsterBefore} -> {MonsterAfter} Context: {Context}",
        ...
        if (result.MonsterDefeated)
        {
            ...
            if (!loot.IsEmpty)
            {
                ...
                _logger.LogInformation("Loot generated for {MonsterName}: {ItemCount} items, {CurrencyTypes} currency types Context: {Context}",
                    monster.Name,
                    loot.Items?.Count ?? 0,
    ...
    {
        _logger.LogTrace("TryDropItem called for item: {ItemName} Context: {Context}", itemName, context);

        if (_currentSession == null)
        {
            _logger.LogWarning("TryDropItem failed: No active game session Context: {Context}", context);
            return (false, "No active game session.");
        }
        ...
        if (currentRoom == null)
        {
            _logger.LogError("TryDropItem failed: Current room is null Context: {Context}", context);
            return (false, "Error: Current room not found.");
        }
        ...
        if (_currentSession.Player.Inventory.TryRemove(item))
        {
            ...
            _logger.LogInformation(
                "Item dropped: {ItemName} in room {RoomName}. Inventory: {InventoryCount}/{Capacity} Context: {Context}",
                item.Name,
                currentRoom.Name,
                _currentSession.Player.Inventory.Count,
                _currentSession.Player.Inventory.Capacity,
    ...
    {
        _logger.LogTrace("TryUseItem called for item: {ItemName} Context: {Context}", itemName, context);

        if (_currentSession == null)
        {
            _logger.LogWarning("TryUseItem failed: No active game session Context: {Context}", context);
            return (false, "No active game session.");
        }
    ...
    {
        _logger.LogTrace("TryUseAbility called for ability: {AbilityName} Context: {Context}", abilityName, context);

        if (_currentSession == null)
        {
            _logger.LogWarning("TryUseAbility failed: No active game session Context: {Context}", context);
            return (false, "No active game session.");
        }
        ...
        _logger.LogInformation(
            "Ability used: {PlayerName} used {AbilityName}. Result: {Message} Context: {Context}",
        ...
        else if (target != null && target.IsAlive)
        {
            ...
            if (combatResult.MonsterCounterAttack != null)
            {
                ...
                _logger.LogInformation(
                    "Monster counterattack: {MonsterName} [{Roll}]+{Mod}={Total} -> {Damage} damage to {PlayerName} Context: {Context}",
    ...
    public TurnEndResult ProcessTurnEnd(string context = "Unspecified")
    {
        ...
        _logger.LogDebug("Turn advanced to {TurnCount} Context: {Context}", newTurnCount, context);
        ...
        foreach (var change in resourceResult.Changes)
        {
            _logger.LogTrace("Processing resource change: {ResourceType} {Old}->{New} Context: {Context}",
                change.ResourceTypeId, change.PreviousValue, change.NewValue, context);
            resourceChanges.Add(MapResourceChange(change));
        }
        ...
        _logger.LogInformation(
            "Turn {Turn} ended: {ResourceChanges} resource changes, {CooldownChanges} cooldown changes, {AbilitiesReady} abilities now ready Context: {Context}",
            newTurnCount, resourceChanges.Count, cooldownChanges.Count, abilitiesNowReady.Count, context);
    ...
    {
        _logger.LogTrace("TryEquipItem called for item: {ItemName} Context: {Context}", itemName, context);

        if (_currentSession == null)
        {
            _logger.LogWarning("TryEquipItem failed: No active game session Context: {Context}", context);
    ...
    {
        _logger.LogTrace("TryUnequipItem called for slot: {Slot} Context: {Context}", slot, context);

        if (_currentSession == null)
        {
            _logger.LogWarning("TryUnequipItem failed: No active game session Context: {Context}", context);
            return new EquipResultDto(false, "No active game session.");
        }
