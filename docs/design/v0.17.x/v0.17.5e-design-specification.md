...
- [ ] `ICharacterFactory` - Application/Interfaces/

### 11.2 Value Objects

- [ ] `FactoryValidationResult` - Domain/ValueObjects/

### 11.3 Services

- [ ] `CharacterFactory` - Application/Services/

### 11.4 Unit Tests

- [ ] `CharacterFactoryTests` (~5 tests)
...
- [ ] `ValidateState()` returns errors for missing required fields
- [ ] `CreateCharacterAsync()` applies all lineage attribute modifiers
- [ ] Clan-Born flexible bonus correctly applied
- [ ] Rune-Marked starts with 5 Corruption
- [ ] Background skills and equipment granted
- [ ] All 6 starting abilities granted (3 archetype + 3 specialization)
- [ ] Derived stats calculated correctly
- [ ] Resources set to maximum values
