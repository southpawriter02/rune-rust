Here is the comprehensive implementation plan for v0.3.21: The Steward (Save Management).
...
To manage the database schema updates and logic complexity, this version is split into two sub-versions:
...
1. Architecture & Data Flow
	...
	•	Extraction: During SaveManager.SaveGameAsync, extract key metrics from GameState and populate the Metadataobject.
...
B. Load Menu Rendering Logic Input: List
	1	Iterate Slots:
	2	Check Metadata:
	◦	Null: Render "Corrupted Save".
	◦	Valid: Render Format: [Slot 1] Ragnar (Lvl 5 Warrior) Location: The Deep Shaft | HP: [Green]██░░░[/] (10/50) Played: 4h 23m
...
B. SaveManager Update File: RuneAndRust.Engine/Services/SaveManager.cs
...
1. Architecture & Data Flow
	...
	◦	Slot -1: Backup 1.
	◦	Slot -2: Backup 2.
	•	Rotation Logic: Whenever an autosave triggers, shift existing saves down the chain before writing the new one. This ensures the player can always revert to a state from ~10-20 minutes ago.
2. Logic Decision Trees
A. Autosave Rotation Logic Input: Trigger Autosave
...
B. Load Menu Integration Input: Load Menu Request
...
3. Code Implementation
A. Repository Rotation Method File: RuneAndRust.Persistence/Repositories/SaveGameRepository.cs
...
Rotation
Debug
"[Save] Rotated autosaves. Deleted slot -2."
-
Save
Metadata
Trace
"[Save] Generated metadata for {Char}. Loc: {Loc}."
Char, Loc
Persistence
Transaction
Verbose
"[DB] Autosave rotation transaction committed."
-

Testing Strategy
Unit Tests (SaveManagerTests.cs):
	•	Metadata_Populated: Create GameState. Call Save. Assert SaveGame.Metadata.CurrentHp matches.
	•	Rotation_Order:
...
Would you like to proceed to v0.3.22: The Tactician II (Combat Polish)?
