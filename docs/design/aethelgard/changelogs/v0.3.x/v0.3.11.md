# Changelog: v0.3.11 - The Archivist (Dynamic Knowledge Engine & Developer's Handbook)

**Versions:** v0.3.11a and v0.3.11b
**Release Date:** 2025-12-23
**Total Tests:** 2595 (25 new tests added across both parts)

## Table of Contents

- [Overview](#overview)
- [Part A: The Living Guide (Dynamic Knowledge Engine)](#part-a-the-living-guide-dynamic-knowledge-engine)
  - [Summary](#summary)
...
- [Part B: The Developer's Handbook](#part-b-the-developers-handbook)
  - [Summary](#summary-1)
...
The v0.3.11 release implements a complete **Documentation Engine** for Rune & Rust, ensuring that in-game documentation stays synchronized with the codebase through reflection-based auto-generation and external Markdown export.

**Part A (The Living Guide)** establishes the Dynamic Knowledge Engine, a reflection-based system that scans the `RuneAndRust.Core` assembly for `[GameDocument]` attributes and generates runtime `CodexEntry` objects. This system populates the Journal's Field Guide tab with documentation extracted directly from enum values, class properties, and type definitions, eliminating documentation drift by design. The implementation includes deterministic GUID generation via MD5 hashing, thread-safe caching, and seamless integration with the JournalService merge pattern.

**Part B (The Developer's Handbook)** extends the Dynamic Knowledge Engine with CLI-based Markdown export functionality. The `DocGenService` leverages `ILibraryService` to retrieve all system-generated entries and exports them as static Markdown files organized by category. The `--docgen` CLI flag triggers documentation generation, creating one file per `EntryCategory` plus a `README.md` index, suitable for external wikis or GitHub documentation.

Together, these releases provide a complete documentation pipeline with 25 new tests (15 for Part A, 10 for Part B), zero build errors, and full integration with existing game systems.

---

## Part A: The Living Guide (Dynamic Knowledge Engine)
...
## Part B: The Developer's Handbook
...
# Run all v0.3.11 tests (both parts)
...
**Test Coverage:** 100% for new services (25/25 tests passing)
