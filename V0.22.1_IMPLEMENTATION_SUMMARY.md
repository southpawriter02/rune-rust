...
- Destruction triggers hazards: `HazardTrigger.OnDestroy`
...
1. **Grid Integration:** Position-based queries are stubs (return empty/center only)
2. **Damage Integration:** Hazard damage is calculated but not applied to characters
3. **Status Effects:** Status effects logged but not applied via service
4. **Terrain Creation:** Logged only, not applied to actual grid
5. **No Database Persistence:** In-memory storage only (as per parent spec)

These are intentional design choices for v0.22.1 scope. Full integration occurs in future phases.
...
### Created
4. `RuneAndRust.Tests/EnvironmentalObjectServiceTests.cs` - Unit tests (23 tests)
5. `RuneAndRust.Tests/EnvironmentalObjectIntegrationTests.cs` - Integration tests (7 scenarios)
6. `V0.22.1_IMPLEMENTATION_SUMMARY.md` - This document
...
The implementation is ready for integration testing with the broader combat system. Stub methods provide clear integration points for future phases.
