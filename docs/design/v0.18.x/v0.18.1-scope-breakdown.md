...
- [x] Corruption accumulates correctly from all sources
- [x] TransferCorruption moves corruption between characters
- [x] Terminal Error check uses WILL vs DC
- [x] Skill modifiers scale with threshold correctly
- [x] ~5 unit tests pass (97 actual test methods, far exceeding the ~5 estimate)
...
- [x] Schema validates correct corruption source definitions
- [x] Schema rejects invalid corruption values
- [x] Configuration loads all corruption source categories
- [x] Database tables created with proper constraints
- [x] Corruption history logged for all changes
- [x] ~3 unit tests pass (18 actual test methods across 11 test sections)
