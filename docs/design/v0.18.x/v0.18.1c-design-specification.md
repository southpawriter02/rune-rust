...
- [ ] `src/Core/RuneAndRust.Application/Interfaces/ICorruptionService.cs`

### Documentation

- [ ] XML documentation on interface and all methods
- [ ] Code examples in interface-level remarks
- [ ] Parameter documentation with exceptions listed
- [ ] Update scope breakdown to mark v0.18.1c complete
...
- [ ] `ICorruptionService` defines all 9 methods as specified
- [ ] All methods have complete XML documentation
- [ ] All parameters have `<param>` tags with descriptions
- [ ] All return types have `<returns>` tags
- [ ] All exceptions have `<exception>` tags
- [ ] Interface includes class-level `<example>` block
- [ ] Interface compiles without implementation references

### Method Signature Criteria

- [ ] `GetCorruptionState` returns `CorruptionState`
- [ ] `GetMaxHpPenaltyPercent` returns `int`
- [ ] `GetMaxApPenaltyPercent` returns `int`
- [ ] `GetResolveDicePenalty` returns `int`
- [ ] `GetSkillModifiers` returns `CorruptionSkillModifiers`
- [ ] `AddCorruption` returns `CorruptionAddResult`
- [ ] `TransferCorruption` returns `CorruptionTransferResult`
- [ ] `RemoveCorruption` returns `bool`
- [ ] `PerformTerminalErrorCheck` returns `TerminalErrorResult`
