...
│  CreationWizardView                                              │
│       │                                                          │
│       ├── [1] LineageScreen ──────────▶ SelectLineageAsync()    │
│       │       • Display 4 lineage options                       │
│       │       • Clan-Born: show attribute selector              │
│       │                                                          │
│       ├── [2] BackgroundScreen ───────▶ SelectBackgroundAsync() │
│       │       • Display 6 background options                    │
│       │       • Show skills/equipment preview                   │
│       │                                                          │
│       ├── [3] AttributeScreen ────────▶ ConfirmAttributesAsync()│
│       │       • Simple/Advanced mode toggle                     │
│       │       • Point-buy controls                              │
│       │       • Live derived stats preview                      │
│       │                                                          │
│       ├── [4] ArchetypeScreen ────────▶ SelectArchetypeAsync()  │
│       │       • Display 4 archetype options                     │
│       │       • ⚠ PERMANENT warning banner                      │
│       │                                                          │
│       ├── [5] SpecializationScreen ──▶ SelectSpecAsync()        │
│       │       • Filtered by archetype                           │
│       │       • Path type indicator                             │
│       │                                                          │
│       └── [6] SummaryScreen ─────────▶ ConfirmCharacterAsync()  │
...
│   ├── Step title
│   └── Progress indicator
├── CurrentStepScreen (ICreationScreen)
│   ├── OptionList OR Custom content
│   └── PreviewPanel
├── NavigationBar
│   ├── [Back] button (if canGoBack)
│   ├── [Cancel] button
...
public class CreationWizardView
{
    ...
    private void RenderStepHeader()
    {
        ...
        _terminal.WriteLineAt(0, 0, "╔" + new string('═', header.Length) + "╗");
        _terminal.WriteLineAt(0, 1, "║" + header + "║");
        _terminal.WriteLineAt(0, 2, "╚" + new string('═', header.Length) + "╝");
...
public class AttributeScreen : ICreationScreen
{
    ...
    public void Render(CharacterCreationViewModel viewModel)
    {
        ...
        for (int i = 0; i < Attributes.Length; i++)
        {
            ...
            Console.WriteLine(
                $"{prefix}{attr,-12} [−]  {value,2}  [+]    {GetAttrDescription(attr)}");
        }
...
public class ArchetypeScreen : ICreationScreen
{
    ...
    public void Render(CharacterCreationViewModel viewModel)
    {
        for (int i = 0; i < Archetypes.Length; i++)
        {
            ...
            Console.WriteLine($"      ⚔ {GetArchetypeStats(arch)}");
            Console.WriteLine($"      🎯 {GetArchetypeAbilities(arch)}");
            Console.WriteLine();
        }
    }
...
| Key          | Context      | Action                    |
| ------------ | ------------ | ------------------------- |
| `↑` / `↓`    | All screens  | Navigate options          |
| `1-9`        | Option lists | Quick select by number    |
| `←` / `→`    | Attributes   | Decrease/Increase value   |
...
**Actor:** Player
**Flow:** Start → Lineage → Background → Attributes → Archetype → Specialization → Summary → Confirm

### UC-02: Navigate Back to Change Selection

**Actor:** Player
**Flow:** At Step 4 → Press B → Returns to Step 3 → Modify attributes → Continue

### UC-03: Cancel Mid-Creation

**Actor:** Player
**Flow:** At any step → Press Escape → Confirm cancel → Return to main menu
...
- [x] `ICreationScreen` - TUI/Screens/ (placed alongside implementations per codebase convention)

### Views

- [x] `CreationWizardView` - TUI/Views/

### Screens

- [x] `LineageScreen` - TUI/Screens/
- [x] `BackgroundScreen` - TUI/Screens/
- [x] `AttributeScreen` - TUI/Screens/
- [x] `ArchetypeScreen` - TUI/Screens/
- [x] `SpecializationScreen` - TUI/Screens/
- [x] `SummaryScreen` - TUI/Screens/

### Components

- [x] `OptionListRenderer` - TUI/Components/
- [x] `StepHeaderRenderer` - TUI/Components/

### Unit Tests

- [x] `CreationScreenTests` (56 tests)
...
- [x] All 6 step screens render correctly
- [x] Navigation (Back/Cancel/Continue) functions correctly
- [x] Archetype step displays PERMANENT warning
- [x] Attribute point-buy updates derived stats live
- [x] Clan-Born shows flexible attribute selector
- [x] Summary displays all selections
- [x] Name input enforces validation rules

### Non-Functional

- [x] All screens have consistent visual style
